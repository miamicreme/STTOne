...
const guidedPath = [
  {
    ...
    intent: 'Frame the business problem before showing technology.',
    next: 'Next: show how discovery starts.'
  },
  {
    label: 'Discovery Map',
    intent: 'Show that the work starts with operators, workflows, and validation.',
    next: 'Next: connect the systems with a simple operating model.'
  },
  {
    label: 'Operating Model',
    intent: 'Explain the architecture in plain English: keep the core systems and govern the handoffs.',
    next: 'Next: show how exceptions are handled.'
  },
  {
...
export function TourController() {
    ...
    return (
      <div className="fixed inset-x-0 bottom-4 z-40 flex justify-center px-4">
        <div className="glass flex items-center gap-3 rounded-2xl border border-emerald-500/25 px-4 py-3 shadow-glow">
          ...
          <span className="text-sm font-medium text-slate-100">Guided path complete — explore freely</span>
          <button
            onClick={startTour}
  ...
  const scene = tourScenes[tour.sceneIndex]
  const path = guidedPath[tour.sceneIndex] ?? guidedPath[0]
  const words = scene?.caption.split(' ') ?? []
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 flex justify-center px-3 pb-3 sm:pb-4">
      <div className="glass relative w-full max-w-4xl overflow-hidden rounded-2xl border border-accent/30 shadow-[0_14px_44px_-12px_rgba(47,134,224,0.45)] ring-1 ring-accent/15">
          ...
          <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
            <span className="flex items-center gap-2">
              ...
              <span className="flex items-center gap-1 text-[10px] font-semibold uppercase tracking-widest text-brand-red-soft">
                <Compass className="h-3 w-3" /> Guided Path {tour.sceneIndex + 1} of {total}
              </span>
            </span>
            <span className="rounded-full border border-accent/25 bg-accent/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-widest text-accent">
              {path.label}
            </span>
          </div>

          <p className="text-[11px] uppercase tracking-[0.18em] text-slate-500">Why this page matters</p>
          <p className="mt-1 text-sm leading-6 text-slate-300">{path.intent}</p>

          <p className="mt-3 min-h-[1.6rem] text-base font-semibold leading-snug tracking-wide text-white sm:text-lg">
            {visibleCaption}
            {isTyping && <span className="ml-1 inline-block h-[1em] w-0.5 animate-pulse bg-accent align-middle opacity-80" />}
          </p>
        </div>

        <div className="border-t border-white/[0.06] bg-base-900/60 px-3 py-2 sm:px-4">
          <div className="mb-2 flex items-center justify-between gap-3 text-[11px] text-slate-500">
            <span>{path.next}</span>
            <span className="tabular">{tour.sceneIndex + 1} / {total}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex shrink-0 items-center gap-1.5">
              ...
              <button
                ...
                aria-label={tour.playing ? 'Pause guided path' : 'Play guided path'}
            ...
            <div className="flex flex-1 items-center gap-1">
              {tourScenes.map((s, i) => (
                <span key={i} className={`relative h-1 flex-1 overflow-hidden rounded-full ${i < tour.sceneIndex ? 'bg-accent/60' : 'bg-white/10'}`}>
                  {i === tour.sceneIndex && (
                    <span
                      key={tour.sceneIndex}
                      className="tour-seg-fill absolute inset-0 rounded-full bg-accent"
                      style={{ animationDuration: `${s.ms}ms`, animationPlayState: tour.playing ? 'running' : 'paused' }}
                    />
                  )}
                </span>
              ))}
