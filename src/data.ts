 ...
 * Demo-only simulation values for the interview prototype.
 */
...
export const executiveKpis: KPI[] = [
  ...
  { label: 'Payroll Sync Health', value: '97.9%', delta: '+0.4%', trend: 'up', hint: 'Paychex to PenguinData' },
...
export const seedExceptions: ExceptionItem[] = [
  { id: 'EXC-1041', title: 'Missing job code — Eureka Fiber Expansion', detail: 'Customer job code not mapped in QuickBooks; billing on hold.', severity: 'high', source: 'QuickBooks', category: 'accounting-map', retryable: true, createdAt: '08:14' },
  { id: 'EXC-1042', title: 'Duplicate worker record — R. Alvarez', detail: 'Two PenguinData profiles resolved to one Paychex worker ID.', severity: 'medium', source: 'PenguinData', category: 'duplicate', retryable: true, createdAt: '08:22' },
  { id: 'EXC-1043', title: 'License class missing — new hire', detail: 'Vehicle eligibility blocked pending CDL class verification.', severity: 'high', source: 'Paychex', category: 'credential', retryable: false, createdAt: '08:39' },
  { id: 'EXC-1044', title: 'ID mismatch — timesheet import', detail: 'Employee ID format mismatch between Drive sheet and Paychex.', severity: 'medium', source: 'Google Drive', category: 'id-mismatch', retryable: true, createdAt: '09:02' },
  { id: 'EXC-1045', title: 'Missing field — direct deposit', detail: 'Routing number absent on onboarding packet.', severity: 'low', source: 'Paychex', category: 'missing-field', retryable: true, createdAt: '09:11' },
  { id: 'EXC-1046', title: 'Accounting map error — vendor bill', detail: 'Trench-unit rental vendor not mapped to expense account.', severity: 'medium', source: 'QuickBooks', category: 'accounting-map', retryable: true, createdAt: '09:27' },
  { id: 'EXC-1047', title: 'Duplicate work orders — Cape Coral', detail: 'Spreadsheet import created 4 duplicate work orders.', severity: 'medium', source: 'PenguinData', category: 'duplicate', retryable: true, createdAt: '09:48' },
  { id: 'EXC-1048', title: 'Missing field — emergency contact', detail: 'New hire packet missing required emergency contact.', severity: 'low', source: 'Paychex', category: 'missing-field', retryable: true, createdAt: '10:03' },
  { id: 'EXC-1049', title: 'ID mismatch — fleet assignment', detail: 'Asset tag F-214 mapped to two crew IDs.', severity: 'medium', source: 'PenguinData', category: 'id-mismatch', retryable: true, createdAt: '10:19' },
  { id: 'EXC-1050', title: 'Missing field — I-9 section 2', detail: 'Document verification fields incomplete.', severity: 'low', source: 'Paychex', category: 'missing-field', retryable: true, createdAt: '10:35' },
  { id: 'EXC-1051', title: 'Accounting map error — small cell', detail: 'Naples small-cell permit cost lacks job-cost class.', severity: 'low', source: 'QuickBooks', category: 'accounting-map', retryable: true, createdAt: '10:52' },
]
...
export const newHireSteps: WorkflowStep[] = [
  ...
  { key: 'drive', label: 'Drive docs matched', detail: 'Certifications and license images linked', system: 'Google Drive' },
  { key: 'profile', label: 'PenguinData profile created', detail: 'Electronic personnel file provisioned', system: 'PenguinData' },
  { key: 'crew', label: 'Crew defaults applied', detail: 'Department, supervisor and crew mapped', system: 'PenguinData' },
  ...
  { key: 'audit', label: 'Audit log committed', detail: 'Immutable onboarding record written', system: 'Audit and Staging' },
]
...
export interface FileVolume { label: string; value: number; tone: 'neutral' | 'warn' | 'good' | 'danger'; hint: string }
export const fileVolumes: FileVolume[] = [
  ...
  { label: 'Migration-Ready', value: 3860, tone: 'good', hint: 'Classified and mapped' },
]

export interface DomainSlice { name: string; value: number; color: string }
...
export interface DuplicateGroup { name: string; copies: number; domain: string; sizeMb: number; recommendation: string }
...
export interface PermissionRisk { scope: string; detail: string; level: ExceptionSeverity; files: number }
...
export interface MigrationDomain { domain: string; target: string; progress: number; files: number }
...
export interface SystemCard { name: string; status: SystemStatus; detail: string; uptime: string; lastSync: string }
export const systemCards: SystemCard[] = [
  ...
  { name: 'PenguinData', status: 'healthy', detail: 'Ops, dispatch and fleet', uptime: '99.91%', lastSync: '1m ago' },
...
export interface ExceptionTaxonomy { category: string; count: number; key: ExceptionCategory }
...
export interface FleetStatus { status: string; count: number; color: string }
...
export interface MaintenanceItem { asset: string; type: string; region: string; dueIn: string; severity: ExceptionSeverity }
export const overdueMaintenance: MaintenanceItem[] = [
  ...
  { asset: 'V-118 Splice Van', type: 'Oil and DOT', region: 'Ohio', dueIn: 'Due tomorrow', severity: 'medium' },
  { asset: 'F-077 Aerial Lift', type: 'Annual cert', region: 'SW Florida', dueIn: 'Due in 3d', severity: 'medium' },
]

export interface Crew { name: string; region: string; lead: string; members: number; assignment: string; status: 'on-site' | 'mobilizing' | 'standby' }
...
export interface Project { name: string; region: string; type: string; complete: number | null; phase: ProjectPhase; status: ProjectStatus; issue: string }
export const projects: Project[] = [
  { name: 'Eureka Fiber Expansion', region: 'Nevada', type: 'Underground', complete: 62, phase: 'build', status: 'blocked', issue: 'Billing hold: customer job code missing in QuickBooks' },
  { name: 'Ohio Neighborhood Upgrade', region: 'Ohio', type: 'Aerial', complete: 81, phase: 'qc', status: 'at-risk', issue: 'Crew assignment gap — supervisor absence' },
  { name: 'Fort Myers MDU Install Program', region: 'SW Florida', type: 'Fulfillment', complete: 91, phase: 'closeout', status: 'at-risk', issue: 'Two new hires pending vehicle qualification' },
  { name: 'Cape Coral Fiber Remediation', region: 'SW Florida', type: 'Repair', complete: 48, phase: 'build', status: 'at-risk', issue: 'Duplicate work orders from spreadsheet import' },
  { name: 'Naples Small Cell Build', region: 'Florida', type: 'Wireless', complete: 37, phase: 'design', status: 'blocked', issue: 'Permit doc still in Drive only' },
  { name: 'Warehouse Asset Baseline', region: 'Fort Myers HQ', type: 'Internal', complete: null, phase: 'build', status: 'active', issue: 'Trailer and trench-unit serial mismatch' },
]
...
export interface RiskRow { region: string; payroll: RiskLevel; fleet: RiskLevel; billing: RiskLevel; docs: RiskLevel; schedule: RiskLevel }
...
export const riskLabels: Record<RiskLevel, string> = { 0: 'Nominal', 1: 'Watch', 2: 'Elevated', 3: 'Critical' }

export type MilestoneStatus = 'done' | 'in-progress' | 'planned'
export interface Milestone { title: string; detail: string; status: MilestoneStatus; quarter: string }
export const transformationRoadmap: Milestone[] = [
  { title: 'Paychex set as HR source of truth', detail: 'Worker master consolidated; duplicate records reconciled.', status: 'done', quarter: 'Q1 2026' },
  { title: 'PenguinData onboarding automation live', detail: 'New-hire pipeline provisions profiles, crews and vehicle checks.', status: 'done', quarter: 'Q1 2026' },
  { title: 'Google Drive audit and classification', detail: '58,412 files indexed; 41,280 classified, 7,936 duplicates flagged.', status: 'in-progress', quarter: 'Q2 2026' },
  { title: 'QuickBooks job-code reconciliation', detail: 'Customer/job-code mapping to clear billing holds.', status: 'in-progress', quarter: 'Q2 2026' },
  { title: 'Legacy Drive migration cutover', detail: 'HR to Paychex, Fleet/Projects to PenguinData, Finance to QuickBooks.', status: 'planned', quarter: 'Q3 2026' },
  { title: 'Unified exception governance', detail: 'Single queue with SLAs across all four systems.', status: 'planned', quarter: 'Q3 2026' },
]

export interface EmployeeProfile { name: string; role: string; employeeId: string; region: string; crew: string; supervisor: string; truck: string; tenure: string; assignment: string }
export const employeeProfile: EmployeeProfile = { name: 'Andre Whitfield', role: 'Fiber Splice Technician', employeeId: 'STT-0418', region: 'SW Florida', crew: 'SW FL Fiber Crew 3', supervisor: 'D. Okafor', truck: 'F-214', tenure: '3.2 yrs', assignment: 'Fort Myers MDU Install Program' }
...
export interface Certification { name: string; status: CertStatus; detail: string }
export const employeeCerts: Certification[] = [
  { name: 'OSHA 10', status: 'valid', detail: 'Valid through Mar 2027' },
  { name: 'Fiber Splicing', status: 'valid', detail: 'Valid through Aug 2026' },
...
export interface EmployeeDoc { name: string; status: DocStatus; detail: string }
...
export interface ShiftTask { time: string; title: string; detail: string; status: ShiftStatus }
export const employeeSchedule: ShiftTask[] = [
  { time: '06:30', title: 'Yard check-in and truck inspection', detail: 'F-214 pre-trip DOT walkaround', status: 'done' },
  ...
  { time: '13:00', title: 'OTDR testing and closures', detail: 'Validate splice loss budget', status: 'upcoming' },
]
export interface TruckDetail { unit: string; type: string; status: SystemStatus; mileage: string; nextService: string; fuel: string }
export const employeeTruck: TruckDetail = { unit: 'F-214', type: 'Bucket Truck — Altec AT40', status: 'warning', mileage: '84,210 mi', nextService: 'Hydraulic inspection overdue 4d', fuel: '5/8 tank' }

export interface LeakageLine { label: string; amount: number; driver: string; system: string }
export const leakageLines: LeakageLine[] = [
  { label: 'Onboarding and data-entry admin', amount: 72000, driver: '126 admin hrs/mo reclaimed', system: 'Paychex to PenguinData' },
  { label: 'Billing leakage — unmapped job codes', amount: 82000, driver: 'Held and mis-coded work across active programs', system: 'QuickBooks' },
  { label: 'Payroll rework and overpayment', amount: 38000, driver: 'Duplicate / mismatched worker records', system: 'Paychex' },
  { label: 'Drive sprawl — storage and retrieval drag', amount: 22000, driver: '7,936 duplicate files plus permission cleanup', system: 'Google Drive' },
]
export const leakageTotal = leakageLines.reduce((sum, line) => sum + line.amount, 0)
...
export const systemsOfRecord: SystemOfRecord[] = [
  ...
  { system: 'PenguinData', owns: 'Operations', role: 'Workforce, dispatch, fleet and work orders' },
  { system: 'QuickBooks', owns: 'Finance', role: 'Invoicing, job-cost and payables' },
  { system: 'Google Drive', owns: 'Documents', role: 'Legacy repository staged and governed' },
]

export type PipelineTrigger = 'webhook' | 'scheduled' | 'event'
export interface PipelineJob { name: string; trigger: PipelineTrigger; flow: string; cadence: string; lastRun: string; throughput: string; health: SystemStatus }
export const pipelineJobs: PipelineJob[] = [
  { name: 'Worker sync', trigger: 'webhook', flow: 'Paychex to PenguinData', cadence: 'Real-time', lastRun: '42s ago', throughput: '118 workers in sync', health: 'healthy' },
  { name: 'New-hire onboarding', trigger: 'event', flow: 'Paychex to Staging to PenguinData to Fleet', cadence: 'On hire', lastRun: '2h ago', throughput: '6 provisioned this week', health: 'healthy' },
  { name: 'Job-cost sync', trigger: 'scheduled', flow: 'PenguinData to QuickBooks', cadence: 'Every 15 min', lastRun: '8m ago', throughput: '214 entries/day', health: 'warning' },
  { name: 'Drive classification', trigger: 'scheduled', flow: 'Google Drive to Staging', cadence: 'Hourly', lastRun: 'streaming', throughput: '58,412 files indexed', health: 'active' },
  { name: 'Dedupe and merge', trigger: 'scheduled', flow: 'Staging to PenguinData', cadence: 'Nightly', lastRun: '06:00', throughput: '7,936 candidates resolved', health: 'healthy' },
  { name: 'Payroll reconciliation', trigger: 'scheduled', flow: 'PenguinData to Paychex', cadence: 'Daily 05:30', lastRun: '05:30', throughput: '97.9% auto-matched', health: 'healthy' },
]

export interface GovernanceGuard { name: string; prevents: string; scope: string }
export const governanceGuards: GovernanceGuard[] = [
  { name: 'Required-field validation', prevents: 'incomplete onboarding packets reaching PenguinData', scope: 'Ingest' },
  { name: 'Dedupe-on-write', prevents: 'duplicate worker, asset and work-order records', scope: 'Staging' },
  { name: 'Schema and format enforcement', prevents: 'malformed IDs and mismatched job codes', scope: 'Ingest' },
  { name: 'Source-of-truth lock', prevents: 'worker edits made anywhere outside Paychex', scope: 'Identity' },
  { name: 'Permission policy', prevents: 'link-public finance docs and stale external access', scope: 'Drive' },
  { name: 'Exception routing', prevents: 'silent failures', scope: 'Pipeline' },
]
